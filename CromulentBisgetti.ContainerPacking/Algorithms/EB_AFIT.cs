...
namespace CromulentBisgetti.ContainerPacking.Algorithms
{
    ...
    public class EB_AFIT : IPackingAlgorithm
    {
        ...
        public AlgorithmPackingResult Run(Container container, List<Item> items)
        {
            ...
            var y1 = itemsToPack[1].CoordY;
            var y2 = itemsToPack[2].CoordY;
            //var y3 = itemsToPack[3].CoordY;
            // var y4 = itemsToPack[4].CoordY;
        ...
        private void ExecuteIterations(Container container)
        {
            ...
            for (int containerOrientationVariant = variantsStart; (containerOrientationVariant <= maxVariants) && !quit; containerOrientationVariant++)
            {
                switch (containerOrientationVariant)
                {
                    case 1://podstawa length x height
                           //     continue;
                        px = container.Length; py = container.Height; pz = container.Width;
                        break;

                    case 2://podstawa length x height - w drugą stronę
                           // continue;
                        px = container.Width; py = container.Height; pz = container.Length;
                        break;

                    case 3://podstawa width x length - !!! :)
                        px = container.Width; py = container.Length; pz = container.Height;
                        break;

                    case 4://podstawa width x height
                           //      continue;
                        px = container.Height; py = container.Length; pz = container.Width;
                        break;

                    case 5://podstawa width x length - w drugą stronę !!! :)
                        px = container.Length; py = container.Width; pz = container.Height;
                        break;

                    case 6://podstawa width x height - w drugę stronę
                           //continue;
                ...
                y1 = itemsToPack[1].CoordY;
                y2 = itemsToPack[2].CoordY;
                //y3 = itemsToPack[3].CoordY;
                //y4 = itemsToPack[4].CoordY;
                ...
                //RŁ if ((container.Length == container.Height) && (container.Height == container.Width)) containerOrientationVariant = 2;
        ...
        private void FindBox(decimal hmx, decimal hy, decimal hmy, decimal hz, decimal hmz)
        {
            ...
            for (y = 1; y <= itemsToPackCount; y = y + itemsToPack[y].Quantity)
            {
                ...
                AnalyzeBox(hmx, hy, hmy, hz, hmz, itemsToPack[x].Dim1_x, itemsToPack[x].Dim2_z, itemsToPack[x].Dim3_y);

                if ((itemsToPack[x].Dim1_x == itemsToPack[x].Dim3_y) && (itemsToPack[x].Dim3_y == itemsToPack[x].Dim2_z)) continue;

                AnalyzeBox(hmx, hy, hmy, hz, hmz, itemsToPack[x].Dim1_x, itemsToPack[x].Dim3_y, itemsToPack[x].Dim2_z);
                AnalyzeBox(hmx, hy, hmy, hz, hmz, itemsToPack[x].Dim2_z, itemsToPack[x].Dim1_x, itemsToPack[x].Dim3_y);
                AnalyzeBox(hmx, hy, hmy, hz, hmz, itemsToPack[x].Dim2_z, itemsToPack[x].Dim3_y, itemsToPack[x].Dim1_x);
                AnalyzeBox(hmx, hy, hmy, hz, hmz, itemsToPack[x].Dim3_y, itemsToPack[x].Dim1_x, itemsToPack[x].Dim2_z);
                AnalyzeBox(hmx, hy, hmy, hz, hmz, itemsToPack[x].Dim3_y, itemsToPack[x].Dim2_z, itemsToPack[x].Dim1_x);
        ...
        private void FindLayer(decimal thickness)
        {
            ...
            for (x = 1; x <= itemsToPackCount; x++)
            {
                ...
                for (y = 1; y <= 3; y++)
                {
                    ...
                    {
                        case 1:
                            exdim = itemsToPack[x].Dim1_x;
                            dimen2 = itemsToPack[x].Dim2_z;
                            dimen3 = itemsToPack[x].Dim3_y;
                            break;

                        case 2:
                            exdim = itemsToPack[x].Dim2_z;
                            dimen2 = itemsToPack[x].Dim1_x;
                            dimen3 = itemsToPack[x].Dim3_y;
                            break;

                        case 3:
                            exdim = itemsToPack[x].Dim3_y;
                            dimen2 = itemsToPack[x].Dim1_x;
                            dimen3 = itemsToPack[x].Dim2_z;
                            break;
                    }
                    ...
                    if ((exdim <= thickness) && (((dimen2 <= px) && (dimen3 <= pz)) || ((dimen3 <= px) && (dimen2 <= pz))))
                    {
                        for (z = 1; z <= itemsToPackCount; z++)
                        {
                            ...
                            {
                                dimdif = Math.Abs(exdim - itemsToPack[z].Dim1_x);

                                if (Math.Abs(exdim - itemsToPack[z].Dim2_z) < dimdif)
                                {
                                    dimdif = Math.Abs(exdim - itemsToPack[z].Dim2_z);
                                }

                                if (Math.Abs(exdim - itemsToPack[z].Dim3_y) < dimdif)
                                {
                                    dimdif = Math.Abs(exdim - itemsToPack[z].Dim3_y);
                                }
        ...
        private void Initialize(Container container, List<Item> items)
        {
            ...
            foreach (Item item in items)
            {
                ...
                {
                    Item newItem = new Item(item.ID, item.Dim1_x, item.Dim2_z, item.Dim3_y, item.Quantity, item.Tag);
        ...
        private void OutputBoxList()
        {
            ...
            itemsToPack[cboxi].Tag = itemsToPack[cboxi].Tag;
